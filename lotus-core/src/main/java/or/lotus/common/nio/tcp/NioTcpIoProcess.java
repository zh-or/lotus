package or.lotus.common.nio.tcp;

import or.lotus.common.support.Utils;
import or.lotus.common.nio.*;
...
//                    long s = System.currentTimeMillis();
    ...
    private boolean handleReadData(ByteBuffer readcache, Session session) {
        ...
        if(ishavepack) {
            session.pushEventRunnable(new IoEventRunnable(msgout.read(), IoEventRunnable.IoEventType.SESSION_RECVMSG, session, context));
            msgout.write(null);
            return remaining > 0;
        }
        return false;
    ...
    private void handleWriteData(NioTcpSession session) {
        ...
        if(msg != null) {
            ...
            /*call message sent*/
            session.pushEventRunnable(new IoEventRunnable(msg, IoEventRunnable.IoEventType.SESSION_SENT, session, context));
        }else{
