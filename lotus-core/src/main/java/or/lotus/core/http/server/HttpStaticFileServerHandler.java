...
public class HttpStaticFileServerHandler extends SimpleChannelInboundHandler<FullHttpRequest> {
    ...
    public void channelRead0(ChannelHandlerContext ctx, FullHttpRequest request) throws Exception {
        if (!request.decoderResult().isSuccess()) {
            LotusResponseSender.sendError(context, ctx, request, BAD_REQUEST);
            return;
        }

        if (!GET.equals(request.method())) {
            LotusResponseSender.sendError(context, ctx, request, METHOD_NOT_ALLOWED);
            return;
        }
        ...
        if (path == null) {
            LotusResponseSender.sendError(context, ctx, request, FORBIDDEN);
            return;
        }


        LotusResponseSender.sendFile(context, ctx, request, path);
    }
    ...
    public void exceptionCaught(ChannelHandlerContext ctx, Throwable cause) {
        ...
        if (ctx.channel().isActive()) {
            LotusResponseSender.sendError(context, ctx, null, NOT_FOUND);
        }
    }
